---
title: "Predicting Self-Harm in Adolescent TBI Patients: R code"
subtitle: "For this project, I developed clinical prediction models to estimate the risk of self-harm following adolescent traumatic brain injury (TBI). This document outlines the key steps in the model development and internal validation, and has been prepared in accordance with [SAIL Databank's](https://saildatabank.com) output review policy. This document is addresses three goals:<br> 
...
```{r, r run_this_one3, eval = TRUE, waring = FALSE, message = FALSE, results='hide'}
...
:::{.callout}
This exclusion criteria was made on the basis that the individuals with zero follow-up time (n = 10) comprised a negligible fraction of the cohort.<br>
...
Information on the outcome of interest is included at this stage. Variable `suicidality_date` contains the date of a diagnosis (defined using ICD-10 or Read Codes) of an injury purposely inflicted by another. Follow-up now needs to finish on the date of self-harm, for those with this outcome.
...
pedw <- sqlQuery(channel, "select * from SAILW1478V.PEDW_VICTIM")
wlgp <- sqlQuery(channel, "select * from SAILW1478V.WLGP_VICTIM")
adde <- sqlQuery(channel, "select * from SAILW1478V.ADDE_VICTIM")
...
test <- temp %>%
  filter(is.na(suicidality_date) & !is.na(second_suicidality_date)) %>%
  dplyr::select(alf, dod, entry_date, end_date, tbi_date, tbi_life, suicidality_date, second_suicidality_date)
```
...
```{r, echo=FALSE}
temp3 <- temp2 %>%
  dplyr::select(-suicidality_life, -suicidality_date, -suicidality_life, -suicidality_date, sud_life, -sud_date, -psychiatric_life, -psychiatric_date, -adhd_life, -adhd_date, -autism_life, -autism_date, -conduct_disorder_life, -conduct_disorder_date, -learning_difficulties_life, -learning_difficulties_date, -neurological_life, -neurological_date, -child_maltreatment_flag, -looked_after_life, -looked_after_date)
...
fakedata <- tibble(
  ...
  exit_dd = 2022.693, 
  age_at_exit = 16.444, 
  suicidality_date = as.Date("2022-09-11"), 
  fail = 1,
  outcome = 1,
  s_day = 0,
  follow_up = 2.445
...
:::{.callout}
If the mean calibration is close to 1 with a narrow CI, this indicates robust calibration. <br> A calibration slope < 1 indicates overestimated risks, while > 1 suggests underestimated risk.
:::
...
:::{.callout}
The lower the brier score the better the model's overall performance. <br> A higher percentage for the scaled Brier score illustrates better model performance: 100% reflecting a perfect model, =0 a useless model, and <0 a harmful model.
:::
